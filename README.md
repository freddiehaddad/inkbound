...
**Automatically maps your tablet to a specific window.**
...
window? If so, then **inkbound** fixes this. It watches your drawing app's
...
inkbound works with [OpenTabletDriver](https://opentabletdriver.net/) (OTD), an
open-source tablet driver that replaces the official Wacom driver. You'll need
to set it up first.
...
inkbound will automatically start the OTD daemon if it's not already running,
and stop it when inkbound exits. If you prefer to manage the daemon yourself,
run `OpenTabletDriver.Daemon.exe` before starting inkbound.
...
Download `inkbound.exe` from the
[Releases](https://github.com/freddiehaddad/inkbound/releases) page, or build
from source:
...
1. **inkbound** finds the matching window and maps your tablet to it
...
6. Press **Ctrl+C** in the terminal to stop inkbound — your original tablet
   mapping is restored
...
```
inkbound.exe --target "krita" --rotation 90
inkbound.exe --target "krita" --rotation 270
```
...
```
inkbound.exe --target "clip studio"
```

### Aspect ratio

Your tablet's physical drawing area has a specific shape (aspect ratio).
inkbound preserves this ratio so your strokes aren't distorted. If the window is
very wide, the tablet will use the full window height but won't stretch to the
edges horizontally — keeping your drawing natural.
...
The application you specified isn't open yet. Open it and inkbound will detect
it automatically.
...
A: Yes — inkbound runs in the terminal. Closing the terminal stops it and
restores your original tablet mapping.

**Q: Can I use this with the official Wacom driver?**
A: No — inkbound requires OpenTabletDriver. The official Wacom driver doesn't
offer a way to change the mapping programmatically.
